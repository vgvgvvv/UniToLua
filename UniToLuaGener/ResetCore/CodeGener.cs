...
namespace ResetCore.CodeDom
{
    public class CodeGener
    {
        ...
        {
            foreach (var str in importNames)
            {
        ...
        {
            foreach (var str in baseTypeNames)
            {
        ...
        /// <returns></returns>
        public CodeGener AddMemberMethod(Type returnType, string name, Dictionary<string, Type> paramDict, MemberAttributes memberAttributes, params CodeStatement[] statements)
        {
            CodeMemberMethod newMethod = new CodeMemberMethod();
            if(returnType != null)
            {
                newMethod.ReturnType = returnType != null ? new CodeTypeReference(returnType) : new CodeTypeReference();
            }

            newMethod.Name = name;
            newMethod.Attributes = memberAttributes;
        ...
        /// <param name="statements"></param>
        /// <returns></returns>
        public CodeGener AddMemberMethod(string returnType, string name, Dictionary<string, Type> paramDict, MemberAttributes memberAttributes, params CodeStatement[] statements)
        {
            CodeMemberMethod newMethod = new CodeMemberMethod();
            if (!string.IsNullOrEmpty(returnType))
            {
                newMethod.ReturnType = !string.IsNullOrEmpty(returnType) ? new CodeTypeReference(returnType) : new CodeTypeReference();
            }
            newMethod.Name = name;
            newMethod.Attributes = memberAttributes;
